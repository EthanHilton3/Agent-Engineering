...
def format_usage_text(model: str, usages: list[ResponseUsage]) -> str:
    """Format single-model usage as plain text, for appending to .txt/.py output files."""
    if not isinstance(usages, list):
        usages = [usages]
    totals = _aggregate_usage([(model, usage) for usage in usages])
    total = totals.get(model, {
